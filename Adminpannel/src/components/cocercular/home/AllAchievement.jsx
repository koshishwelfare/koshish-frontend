...
const AllAchievement = () => {
  const { getAchievement, handelgetAchievement } = useContext(CocirculerContext);
  const navigate = useNavigate();
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState('newest');
  const [filter, setFilter] = useState('');
  const [achievementData, setAchievementData] = useState(getAchievement || []);

  useEffect(() => {
    if (handelgetAchievement) {
      handelgetAchievement();
    }
  }, []);

  useEffect(() => {
    setAchievementData(getAchievement || []);
  }, [getAchievement]);
  ...
  const filteredData = useMemo(() => {
    ...
    return data;
  }, [achievementData, search, sort, filter]);
  ...
  const columns = [
    { key: 'heading', label: 'Achievement Title' },
    {
      key: 'description',
      label: 'Description',
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex gap-2">
          <button
            onClick={() => navigate(`/landpage/achievement/update/${row._id}`)}
          ...
          <button
            onClick={() => navigate(`/landpage/achievement/${row._id}`)}
            className="admin-btn admin-btn-secondary p-2 flex items-center gap-1"
            title="Edit"
          >
            <FaEdit size={14} />
  ...
  return (
    <div className="space-y-6">
      <ListPageHeader
        ...
        addButtonPath="/landpage/achievement/add"
        searchPlaceholder="Search by achievement title..."
      />
